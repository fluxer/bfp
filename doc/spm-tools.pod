...
Binary tarballs can be uploaded via FTP to mirrors via upload mode. Note that
it does not handle secure connections yet.
...
optional arguments:
  ...
  -s, --signatures      Check for signature(s) available but on in the sources
  -a, --all             Perform all checks
