...
=head1 SYNOPSIS
    ...
    spm-tools [-h] [--debug] [--version]
                 {dist,check,clean,lint,sane,merge,edit,which,pack,pkg,serve,
                 disowned,online,upgrade,digest,portable}

=head1 DESCRIPTION

Source Package Manager Tools (SPMT) packs additional utilities to assist in the
system maintenance. It supports plugins the location of which is
/etc/spm/plugins. They are automatically loaded and executed, if requested via
argument. For example plugin see @DOCDIR@/bfp.
...
Package files from CRUX Linux can be obtained via pkg mode, later they can be
...
Checking if the system can access the internet can be done via online mode.

Migrating local targets metadata from old SPM version to newer can be done via
upgrade mode. It is safe to run multiple times and is valuable only on host
upgrading from SPM v1.7.x. It is available on versions newer than SPM v1.7.x.

Create targets footprint checkum and later verifying it can be done via digest
mode. It uses sha256 digest and creates a digest.json in a designated
directory.
...
positional arguments:
  {dist,check,clean,lint,sane,merge,edit,which,pack,pkg,serve,disowned,online
  ,upgrade,digest,portable}
...
optional arguments:
  ...
  -c, --checksums       Check for checksum(s) not in the sources array
...
optional arguments:
  ...
  -d DIRECTORY, --directory DIRECTORY
                        Set output directory
  -c, --create          Create digest
  -v, --verify          Verify digest
  -k, --backup          Do not ignore backup files
...
Get package files from CRUX for zlib and convert them for use with SPM:

    spm-tools pkg zlib && pkg2src zlib
...
Check if http://crux.nu is accessible from the current host:

    spm-tools online -u http://crux.nu

Upgrade from old SPM version:

    sudo spm-tools upgrade

Create a digest.json file for bash and zlib with their files checksumed:

    spm-tools digest -c bash zlib

Verify bash and zlib checksums from digest.json on extenral device mounted at
/mnt/usb:

    spm-tools digest -v bash zlib -d=/mnt/usb

Create an archive of neverball and all of its dependencies to be run on
different host:

    sudo spm-tools portable neverball 
