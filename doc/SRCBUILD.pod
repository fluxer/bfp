...
(2) more functions can be specified (src_compile and src_check) and six (6)
more variable (depends, makedepends, checkdepends, source, options and backup).
Target name is defined by the directory name in which the target files are
...
For easy of use SOURCE_DIR and INSTALL_DIR are exported and can be used inside
the SRCBUILD.
