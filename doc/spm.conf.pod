...
Default: .*/.packlist|.*/perllocal.pod|.*/share/info|.*/share/gtk-doc|.*.la
...
=head2 IGNORE_OWNERSHIP

Set whether to ignore ownership of files and directories. This increases the
security by making sure that they are not owned by non-system user (usually
with GID/UID that is greater or equal to 1000). By setting it to True you open
up a chance for non-system users to temper with paths that they should not.
...
spm(8) SRCBUILD(5) gpg(1) bash(1) gcc(1) ld(1) make(1) objcopy(1) strip(1)
