...
Set a key ID to be used for signing files such as tarballs. If empty files are
not signed.
...
Default: .*/.packlist|.*/perllocal.pod|.*/share/info/dir|.*/share/gtk-doc
...
limited storage you may want to keep that set to False.
...
the chance for exploiting them but it may corrupt some of them (very rare).

Default: True

=head2 COMPRESS_BIN

Set whether to compress binaries via UPX. Doing so will save you some space
at the cost of system memory. Expect failures during software installation
phase as UPX is quite picky, read its documentation to learn more about it.
...
spm(8) SRCBUILD(5) gpg(1) bash(1) gcc(1) ld(1) make(1) objcopy(1) strip(1) upx(1)
