...
=head1 SYNOPSIS
	...
	spm [-h] [--cache CACHE] [--build BUILD] [--root ROOT] [--gpg GPG]
           ...
           [--version]
           {repo,remote,source,binary,local,cache,who} ...
...
internally the files content (metadatas and SRCBUILDs) in a single JSON file 
which is loaded into the memory and rebuilds the caches when needed to reduce
the lookup overheath. The cache itself is build only when needed too (on the
first lookup) in a JIT (Just In Time) fashion.
...
which means an attacker can exploit this and modify the file content and
feeding it to SPM. As a protection SPM supports checking OpenPGP signatures
when available but that is up to the host to provide and packager to support
by adding the singature URL to the sources array in the SRCBUILD.
...
if, for whatever the reason, SPM chokes you can decompress the tarball with:
	...
	sudo rm -f /var/cache/spm/local.json
...
positional arguments:
  ...
  cache                 Regenerate local and/or remote metadata caches
...
optional arguments:
  -h, --help            Show this help message and exit
  -r, --remote          Regenerate remote metadata cache
  -l, --local           Regenerate local metadata cache
  -a, --all             Short for remote and local
...
Regenerate both remote and local targets global caches:
	sudo spm cache -a
...
Note that all modes, except source, repo and cache, accept regexp similar to
