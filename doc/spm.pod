...
cache itself is build only when needed too (on the first lookup) in a JIT (Just
In Time) fashion. It does not use locks and multiple instances can be run at
the same time, that is something that few package manager support.
