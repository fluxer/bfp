...
=head1 SYNOPSIS
  ...
  spm [-h] [--cache CACHE] [--build BUILD] [--root ROOT] [--gpg GPG]
    [--shell SHELL] [--ignore IGNORE] [--sign SIGN]
    ...
    [--static {True,False}] [--rpath {True,False}] [--upx {True,False}]
    ...
    [--version]
    {repo,remote,source,binary,local,who,aport} ...
...
updates on them. Using Content Version System (Git or SVN) allows going back
and forth in the history of the repository thus building older version of
software would be as easy as checking out the right change set and then issuing
the usual build command via SPM.
...
features but some of those may be implemented later on.
...
positional arguments:
  ...
  aport               Automatically write SRCBUILD for sources given via URL
...
optional arguments:
  ...
  --split {True,False}  Set whether to split debug symbols from binaries and libraries
  --binaries {True,False}
  ...
  --upx {True,False}    Set whether to compress binaries
...
optional arguments:
  ...
  -B, --buildmissing    Build from source when binary not available
...
positional arguments:
  URLS                  URLs to apply actions on

optional arguments:
  -h, --help            Show this help message and exit
  -d DIRECTORY, --directory DIRECTORY
                        Set output directory
  -a, --automake        Build the target after analizing and creating SRCBUILD
                        for it
...
For more information visit http://docs.python.org/2/library/re.html. Source and
binary mode accepts base (e.g. foo), directory (e.g. /home/me/foo) with
SRCBUILD in it, aliases (e.g. base) which you can find on the top-level of a
repository as foo.alias or finally meta groups - currently only two - world and
system. Aport mode accepts URL that is one of Git, SVN (Subversion), rsync,
HTTP(S) or FTP(S).
