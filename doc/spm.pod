...
missing Python runtime library for whatever the reason. Do note that SPM
caches internally the database files in memory and rebuilds the caches when
needed to reduce the lookup overheath.
...
Upon targets merge or removal triggers for updating manual pages, desktop
files, mime and other database will be activated to take care of common actions
