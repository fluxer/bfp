...
detect the RAM filesystem image created by this tool, including recovery images.
Nor is it tide to specific initialization system, unlike many other tools out
there with the same goal. Also the implementation is dead simple (from users
perspective) - a few files to control the modules and files that will be
included in the image along with hooks support for special needs such as
encrypted partitions.

The bad thing is that it requries Python to create images which for such a tool
may be an overkill but it was written to go along Source Package Manager.

It can strip binary/library (ELF) files for smaller images and create recovery
image with all of the modules the kernel chosen or detected provides which is
usefull in case the hardware configuration changes and additional modules are
...
lsinitfs(8) scanelf(1) gzip(1) cpio(1) modprobe(8), spm(8)
