...
WindowState = namedtuple('State', 'maximized_vert, maximized_horz, undecorated, urgent, hidden, fullscreen, skip_taskbar, skip_pager')
'''Window state
...
skip_taskbar
  Is window hidden from taskbar

skip_pager
  Is window hidden from pager
'''
...
class WM(object):
    ...
    def keycode(self, key):
        ...
        if sym is X.NoSymbol:
            message.critical('Invalid key [%s]' % key)
            return None
    ...
    def bind_key(self, window, keydef):
        ...
        for m in mod:
            try:
                ...
                message.critical('Invalid key [%s]' % keydef)
    ...
    def get_window_icon(self, window):
        """Return _NET_WM_ICON_NAME property"""
        d = self.get_window_property_safe(window, '_NET_WM_ICON_NAME', self.get_atom('UTF8_STRING'))
        if d is None or d.format != 8:
            return None
        else:
            return d.value
    ...
    def get_window_state(self, window):
        ...
        return WindowState(
            ...
            state and self.get_atom('_NET_WM_STATE_HIDDEN') in state.value,
            state and self.get_atom('_NET_WM_STATE_FULLSCREEN') in state.value,
            state and self.get_atom('_NET_WM_STATE_SKIP_TASKBAR') in state.value,
            state and self.get_atom('_NET_WM_STATE_SKIP_PAGER') in state.value,
        )
    ...
    def stop(self, is_exit=False):
        ...
        for h in self.deinit_handlers:
            try:
                ...
                message.critical('Shutdown error')
                raise
