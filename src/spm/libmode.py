...
class Repo(object):
    ...
    def sync(self):
        ''' Sync repository '''
        rdir = os.path.join(libconfig.CACHE_DIR, 'repositories')
    ...
    def main(self):
        ...
        for repository in self.repositories_urls:
            ...
            self.repository_dir = os.path.join(libconfig.CACHE_DIR, 'repositories', self.repository_name)
...
class Source(object):
    ...
    def __init__(self, targets, do_clean=False, do_prepare=False,
        ...
        self.mirror = libconfig.MIRROR
        self.strip_binaries = libconfig.STRIP_BINARIES
        self.strip_shared = libconfig.STRIP_SHARED
        self.strip_static = libconfig.STRIP_STATIC
        self.strip_rpath = libconfig.STRIP_RPATH
        self.compress_man = libconfig.COMPRESS_MAN
        self.ignore_missing = libconfig.IGNORE_MISSING
    ...
    def set_global(self, target):
        ...
        self.source_dir = os.path.join(libconfig.BUILD_DIR, self.target_name, 'source')
        self.install_dir = os.path.join(libconfig.BUILD_DIR, self.target_name, 'install')
        ...
        self.sources_dir = os.path.join(libconfig.CACHE_DIR, 'sources', self.target_name)
        self.target_tarball = os.path.join(libconfig.CACHE_DIR, 'tarballs',
            self.target_name + '_' + self.target_version + '.tar.bz2')

    def update_databases(self, content):
        ''' Update common databases '''
        if not libconfig.TRIGGERS:
            return
        ...
        for sfile in sorted(content):
            sfile = '/' + sfile
            if sfile.endswith('.so') and os.path.isfile(os.path.join(libconfig.ROOT_DIR, 'etc/ld.so.conf')) \
                and run_ldconfig:
                message.sub_info('Updating shared libraries cache')
                message.sub_debug(sfile)
                subprocess.check_call((misc.whereis('ldconfig'), '-r', libconfig.ROOT_DIR))
                run_ldconfig = False
    ...
    def remove_target_file(self, sfile):
        ''' Remove target file '''
        sfull = (os.path.join(libconfig.ROOT_DIR + sfile))
    ...
    def remove_target_dir(self, sdir):
        ...
        sfull = (os.path.join(libconfig.ROOT_DIR + sdir))
    ...
    def install(self):
        ...
        if not os.path.isdir(os.path.join(libconfig.CACHE_DIR, 'tarballs')):
            os.makedirs(os.path.join(libconfig.CACHE_DIR, 'tarballs'))
    ...
    def merge(self):
        ...
        if target_upgrade and libconfig.SCRIPTS \
        ...
        misc.archive_decompress(self.target_tarball, libconfig.ROOT_DIR)
        ...
        if target_upgrade:
            ...
            for sfile in remove_content:
                ...
                if libconfig.LOCAL_DIR in sfile:
                    continue
                ...
                sdir = os.path.join(libconfig.ROOT_DIR, os.path.dirname(sfile))
    ...
    def remove(self):
        ...
        if misc.file_search('\npre_remove()', self.srcbuild, escape=False) and libconfig.SCRIPTS:
            message.sub_info('Executing pre_remove()')
            misc.system_script(self.srcbuild, 'pre_remove')

        footprint = os.path.join(libconfig.ROOT_DIR, self.target_footprint)
        ...
        if database.local_installed(self.target_name):
            ...
            misc.dir_remove(os.path.join(libconfig.LOCAL_DIR, self.target_name))
    ...
    def main(self, targets=None, automake=False):
        ...
        for target in targets:
            ...
            if misc.string_search(target_name, libconfig.IGNORE, exact=True):
            ...
            if self.do_compile or automake:
                if not misc.file_search('\nsrc_compile()', self.srcbuild, escape=False):
                    ...
                    os.putenv('CHOST', libconfig.CHOST)
                    os.putenv('CFLAGS', libconfig.CFLAGS)
                    os.putenv('CXXFLAGS', libconfig.CXXFLAGS)
                    os.putenv('CPPFLAGS', libconfig.CPPFLAGS)
                    os.putenv('LDFLAGS', libconfig.LDFLAGS)
                    os.putenv('MAKEFLAGS', libconfig.MAKEFLAGS)
                    self.compile()
